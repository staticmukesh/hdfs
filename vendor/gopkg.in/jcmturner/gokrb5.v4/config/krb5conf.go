...
func newLibDefaults() *LibDefaults {
	...
	// Quick fix for cases where user.Current() returns nil. (Seems to be the case when the user is root?)
	uid := "0"
	homeDir := "/root"
	if usr != nil {
		uid = usr.Uid
	...
	return &LibDefaults{
		...
		DefaultClientKeytabName: fmt.Sprintf("/usr/local/var/krb5/user/%v/client.keytab", uid),
